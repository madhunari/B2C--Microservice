...
import com.example.Exceptions.CartSaveFailedException;
...
public class CartResource {
	...
	public ResponseEntity<CommonApiResponse> addToCart(CartRequestDto request) {
	  ...
	  response.setResponseMessage("missing input"); response.setSuccess(false);
	  ...
	  if (cart == null) { response.setResponseMessage("Cart not found:(");
	  response.setSuccess(false);
	  
	  return new ResponseEntity<CartResponseDto>(response, HttpStatus.BAD_REQUEST);
	  }
	  
	  try { if (request.getQuantity() == 0) { this.cartService.deleteCart(cart); }
	  else { cart.setQuantity(request.getQuantity());
	  
	  this.cartService.updateCart(cart); } } catch (Exception e) { throw new
	  CartSaveFailedException("Failed to update the Cart"); }
	  ...
	  return new ResponseEntity<CartResponseDto>(response, HttpStatus.OK);
		...
		private BigDecimal calulateTotalAmountFromCart(List<Cart> carts) {
		  ...
		  try { this.cartService.deleteCart(cart); } catch (Exception e) { throw new
		  CartSaveFailedException("Failed to delete the Cart"); }
